...
- rrweb 기반 self-hosted 세션 녹화·재생, 검색, 삭제, 보존기간 정리
...
이후 `http://127.0.0.1:3200/d/multi-project-logs`에서 로그 대시보드를, `http://127.0.0.1:3210`에서 리플레이를 연다. 두 화면의 사용자명과 무작위 생성 비밀번호는 로컬 `.env`에만 저장되며 Git에는 포함되지 않는다.
